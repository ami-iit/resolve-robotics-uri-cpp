...
std::optional<std::string> resolveRoboticsURI(const std::string& uriFilename, std::string& errorMessage)
{
  ...
  if (uriFilename.substr(0, packageUriPrefix.size()) != packageUriPrefix &&
      modelUriPrefix.substr(0, modelUriPrefix.size()) != modelUriPrefix)
    {
        ...
        errorMessage = "ResolveRoboticsURICpp: URI " + uriFilename + " is not a existing file path and does not start with file:/, package:/ or model:/, it is not possible to resolve it.";
        return {};
    }
  ...
  std::vector<std::string> envListShare = {"ROS_PACKAGE_PATH", "GAZEBO_MODEL_PATH", "SDF_PATH", "IGN_GAZEBO_RESOURCE_PATH", "GZ_SIM_RESOURCE_PATH"};
